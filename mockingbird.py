...
parser.add_argument('--target_pool',
                    ...
                    type=int,
                    default=1,
                    help='number of samples in the target pool.')
parser.add_argument(
    '--num_iteration',
    required=True,
    type=int,
    help='number of iterations to generate the adversarial trace.')
...
X_train = X_train[:, :
                  num_bursts]  # pick the first 'number of bursts' in each trace
...
X_train = X_train.reshape(
    (X_train.shape[0], 1, X_train.shape[1],
     1))  # reshape the data to be compatible with CNN input
...
if args.detector == 'DF':
    ...
    OPTIMIZER = Adamax(lr=learning_rate,
                       ...
                       decay=0.0)
    K.image_data_format()
    model = mb_utility.ConvNet.build(input_shape=input_shape,
                                     classes=num_classes)
...
if args.detector == 'AWF':
    ...
    K.image_data_format()
    model = mb_utility.AWFConvNet.build(input_shape=input_shape,
                                        classes=num_classes)
    model.compile(loss="categorical_crossentropy",
                  optimizer=OPTIMIZER,
                  metrics=["accuracy"])
...
dist_tf = tf.sqrt(
    tf.reduce_sum(tf.square(x_tf - target_tf),
                  list(range(1, len(X_train.shape)))))
...
for case in cases:
    ...
    X_adv = [
    ]  # a list for keeping the generated defended samples for this config
    ...
    for source in range(num_classes):
        ...
        for i in range(len(X_source)):  # for each sample in source class
            ...
            ind = np.random.randint(0,
                                    high=len(X_others),
                                    size=args.target_pool)
            X_others_samples = X_others[
                ind]  # contains the selected target samples
            ...
            dist = distance(
                X_sample_new, X_others_samples, feat
            )  # distance between the source and selected target samples
            min_dist = np.argmin(
                dist
            )  # contains the index of the minimum distance between the source trace and all the target traces
            max_dist = np.argmax(
                dist
            )  # contains the index the maximum distance between the source trace and all the target traces
            ...
            for k in range(iterations):
                ...
                X_sample_new[ind] = X_sample_new[ind] * (
                    1 + alpha * derivative[ind])

                # Get how our model predict the modified source traces, and how much its confidence in the source class
                ypredict, source_prob = test_classification(model,
                                                            args.detector,
                                                            X_sample_new,
                                                            source,
                                                            sess=sess)
                ...
                if change_applied < confidence_threshold and (
                        steps % 10 == 0):  # drop the target and pick a new one
                    ...
                    min_dist = np.argmin(
                        dist)  # contains the index of min distance
                    max_dist = np.argmax(
                        dist)  # contains the index of max distance
    ...
    overhead = (np.sum(np.abs(X_adv)) - np.sum(np.abs(X_test))) / np.sum(
        np.abs(X_test))
    ...
    case_end = time.time()
    print('Adversarial Trace Generation is done in ',
          (case_end - case_start) / 60, 'mins.')
sess.close()
...
for case in cases:
    ...
    Y_adv = np_utils.to_categorical(
        Y_adv, num_classes)  # change numerical Y values to categrical
    ...
    X_adv_expand = mb_utility.expand_me(X_adv_rescaled,
                                        input_length=input_length)
    # reshape it to be compatible to CNN
    X_adv_expand = X_adv_expand.reshape(
        (len(X_adv_expand), 1, input_length, 1))
    ...
    raw_adv_file = save_path + '{}_{}_adversarial_trace_case_{}'.format(
        args.detector, args.data_type, str(case))
